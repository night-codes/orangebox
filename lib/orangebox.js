...
function app () {
	...
	function _get (name) {
		...
		name = name.toLowerCase();
		return this.variables ? this.variables[name] : params[name];
	}
	...
	function _alias (alias, path) {
		...
		if (!listened) {
			waiters.push({cmd: 'alias', alias: alias, path: path});
		} else {
	...
	function _fileServer (path, routePath) {
		routePath = routePath || '';
		if (!listened) {
			waiters.push({cmd: 'fileServer', path: path, routePath: routePath});
		} else {
			workers.forEach (function(el) {
				if (el) {
					el.send();
