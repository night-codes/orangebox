...
function app () {
	...
	var Server = function(request, response, next) {
		while (fileservers.length) {
			var serv = fileservers.shift();
		...
		Router[el] = function(){
			...
			for (var i = 0; i < arguments.length; i++) {
				if(typeof arguments[i] == 'function') {
					if (arguments[i].fileservers && arguments[i].fileservers.length) {
						while (arguments[i].fileservers.length) {
							fileservers.push(arguments[i].fileservers.shift());
						}
					}
					routes.push({
						route: route,
	...
	function _type (type) {
		var response = this;
		response.set('Content-Type', mime.lookup(type));
		return response;
	}
	...
	function _fileServer (path, routePath) {
		routePath = routePath || '';
		if (!listened) {
			fileservers.push({cmd: 'fileServer', path: path, routePath: routePath});
		} else {
			workers.forEach (function(el) {
				if (el) {
					el.send();
